...
export default function KanbanBoard({initialColumns = [], initialRules = []}: KanbanBoardProps) {
                ...
                if (sourceColIndex !== -1 && targetColIndex !== -1) {
                    ...
                    if (taskIndex !== -1) {
                        const taskToMove = {
                            ...
                            status: newColumnsAfterAutomation[targetColIndex].title
                        };
    ...
    const addTask = (columnId: string, taskData: Omit<Task, 'id' | 'createdAt' | 'status'> & {
        priorityValue?: TaskPriority | 'none_explicit',
    ...
    // 4.2.9 Hàm deleteTask(taskId) được thực thi
    // 4.2.10 Lưu thay đổi vào database
    // 4.2.11 Hiển thị thông báo (toast)
    // 4.2.12 Xoá task được chọn: setSelectedTask(null).
    // 4.2.13 Giao diện TaskDetailSidebar đóng lại và giao diện được cập nhật.
            ...
            <div className="flex gap-4 h-full overflow-x-auto py-2">
                ...
                {filteredColumns.map((column) => (
                    <Column
                        key={column.id}
                        column={column}
                        onAddTask={(colId, taskData) => addTask(colId, taskData as Omit<Task, 'id' | 'createdAt' | 'status'> & {
                            priorityValue?: TaskPriority | 'none_explicit',
                            assigneeValue?: string
                        })}
                ...
                {!isLoadingFilters && filteredColumns.length === 0 && (
                    <div
                        className="flex-1 flex flex-col items-center justify-center h-full text-center text-muted-foreground">
        ...
        <div className="flex flex-col h-full min-h-screen p-4 md:p-6 space-y-4 md:space-y-6 bg-background">
            <header
                ...
                <div className="flex-grow w-full">
                    ...
                    <Tabs defaultValue="board" className="w-full h-full flex flex-col" value={activeTab}
                        ...
                        {activeTab === 'board' && (
                            <div
                                className="p-3 md:p-4 my-2 flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 items-center bg-card border dark:border-neutral-700 rounded-lg shadow-sm">
                                Search Keyword
                                        ...
                                        <SelectContent>
                                            ...
                                            {(columns || []).map(column => (
                                                <SelectItem key={column.id}
                                                            value={column.id}>{column.title}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
